...
const PublicPostsRoute = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(publicQuery, async (snapshot) => {
      ...
      for (let post of allPublicPosts) {
        const commentsQuery = collection(doc(database, 'public', post.id), 'comments');
              ...
              <View style={styles.postHeader}>
                ...
                <View style={styles.postUser}>
                  <TouchableOpacity onPress={() => {
                    if (item.uid === userId) { navigation.navigate('Profile'); } else {
...
const PrivatePostsRoute = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(privateQuery, async (snapshot) => {
      ...
      for (let post of allPrivatePosts) {
        const commentsQuery = collection(doc(database, 'private', post.id), 'comments');
        const commentsSnapshot = await getDocs(commentsQuery);
        post.commentsCount = commentsSnapshot.size;
      }
...
const QuestionsRoute = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(questionQuery, async (snapshot) => {
      ...
      for (let post of allQuestionPosts) {
        const commentsQuery = collection(doc(database, 'questions', post.id), 'comments');
        const commentsSnapshot = await getDocs(commentsQuery);
        post.commentsCount = commentsSnapshot.size;
      }
